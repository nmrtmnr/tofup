...
import {ESBuildOptions, send, UserConfig} from "vite";
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (fullReqPath.endsWith(".html")) {
          ...
          if(!fs.existsSync(pugPath)){
            // DynamicRoutingをチェック
            const regexp = /.*\[.*\].*\.pug/;
            const dynamicFiles = glob.sync(path.join(projectRoot, "**/*.pug"), {
              nodir: true
            ...
            if (targetData) {
              for (const targetDatum of targetData) {
                ...
                let result = d.filter((item: { [index: string]: any }) => {
                  return item[targetDatum.match] === targetDatum[targetDatum.match];
                })[0];
                ...
                const html = await transformPugToHtml(server, targetDatum.pug, options);
