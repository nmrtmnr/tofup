...
import {getBetweenStr, getRouterFile, isDynamicName} from "./util/checkDynamic.js";
...
export const vitePluginPugBuild = (options: PugViteOptions): Plugin => {
  ...
  return {
    ...
    async options(inputOptions) {
      ...
      const dynamicFiles = glob.sync(globbingPath, {
        nodir: true
    ...
    resolveId(source: string, a, v) {
      if (source.endsWith(".pug")) {
        ...
        if (!slug) {
          ...
          const dummy = `${
              source.slice(0, Math.max(0, source.lastIndexOf("."))) || source
          }.html`.replace(options?.view, "").replace(`[${slug}]`, data.data[data.done][slug]);
          pathMap[dummy] = {
            source,
    ...
    load(id: string) {
      if (id.endsWith(".html")) {
        if (pathMap[id]) {
          const html = compileFile(pathMap[id].source, options?.options)(deepmerge(options.data || {},pathMap[id].data));
          return html;
        }
